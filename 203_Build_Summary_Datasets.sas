...
proc logistic data=casuser.Mainbearingsummarystats&t (drop=targetX targetGB&xa Targetgb&xb tdp_asset_name) descending outest=betas outmodel=sato.predGB&t;
   class targetGB&t ;
   model targetGB&t = tdp: / selection=backward slstay=&stay.;
   *output out=pGB&t;
run;
proc logistic inmodel=sato.predGB&t;
   score data=casuser.MainbearingSummaryStats&t out=temp (keep=tdp_asset_name targetGB&t p_1);
run;
