...
proc means data=sato.mbads1008 nonobs n sum;
   class unit_name;
   var flg_mb_issue;
run;
proc means data=sato.mbads4320 nonobs n sum;
   class targetX unit_name;
   var flg_mb_issue;
run;

proc means data=sato.mbads4320 nonobs n sum nway;
...
proc freq data=sato.ml_abt_23May2021; table flg_mb_issue*unit_name; run;
proc freq data=sato.ml_abt_23May2021; table unit_name*flg_mb_issue*flg_mb_failure; run;


*** Note: there are only MB Issues when MB failure is "1" ***;
data EventNotes;
   set sato.mbads4320(keep=unit_name tdp_asset_name flg_mb_failure flg_mb_issue erm_fault_notes targetX);
   where unit_name='BG2' and flg_mb_failure=0;
   *if flg_mb_issue=1;
run;
proc freq; table unit_name*flg_mb_issue*flg_mb_failure; run;
